    ...
    <div class="product__wrapper bg-white flex-col md:flex-row  flex justify-between py-4 px-3 pr-3 md:pr-8">
        <div class="product__gallery mb-5 md:mb-0 w-full  mr-0 md:mr-6 lg:mr-12 xl:mr-20">
            ...
            <div class="main__switcher grid grid-cols-5">
                <div class="elem__switch current__switch cursor-pointer inline-flex items-center justify-center"
                    data-switch="{{ asset('img/mainimage.webp') }}">
                    <img src="{{ asset('img/mainimage.webp') }}" alt="mainimage">
                </div>
                <div class="elem__switch cursor-pointer inline-flex items-center justify-center"
                    data-switch="{{ asset('assets/img/mainimage2.webp') }}">
                    <img src="{{ asset('img/mainimage2.webp') }}" alt="mainimage">
                </div>
                <div class="elem__switch cursor-pointer inline-flex items-center justify-center"
                    data-switch="{{ asset('img/mainimage3.webp') }}">
                ...
                <div class="elem__switch cursor-pointer inline-flex items-center justify-center"
                    data-switch="{{ asset('img/mainimage2.webp') }}">
                    <img src="{{ asset('img/mainimage2.webp') }}" alt="mainimage">
                </div>
                <div class="elem__switch cursor-pointer inline-flex items-center justify-center"
                    data-switch="{{ asset('img/mainimage3.webp') }}">
        ...
        <div class="product__description w-full">
            ...
            <div class="content__description mb-4">
                <p class="text-xs light__grey">La Dodge Charger SXT AWD est une bête. Je l'ai depuis près de six ans
                    maintenant, et je l'ai conduite sur les routes de montagne, en ville et même à la mort une fois.
                    C'est une excellente voiture polyvalente qui peut gérer tout ce que vous lui lancez.</p>
            </div>
            <div class="grid__description grid grid-cols-2 gap-x-10">
                <div class="row__wrapper">
                    ...
                    <div class="column__description flex items-center justify-between mb-2">
                        ...
                        <span
                            class="text-sm light__grey text-right">{{ \Carbon\Carbon::parse($car->created_at)->format('M Y') }}</span>
                    </div>
                    <div class="column__description flex items-center justify-between mb-2">
                        <p class="m-0 text-sm font-medium dark__grey text-left mr-2">Kilométrage</p>
                        <span class="text-sm light__grey text-right">{{ number_format($car->mileage , 2) }} Km</span>
                    </div>
                ...
                <div class="row__wrapper">
                    ...
                    <div class="column__description flex items-center justify-between">
                        ...
                        <span
                            class="text-sm light__grey text-right">{{ \Carbon\Carbon::parse($car->date_car)->format('Y') }}</span>
                    </div>
                </div>
            </div>
            <form action="" class="mt-7 w-full">
                <div class="bid__buttons w-full grid grid-cols-1 gap-y-3  gap-x-3 xl:gap-x-7">
                    <!-- <a href="#" class="outline-btn inline-flex items-center justify-center font-medium text-sm">{{ number_format($car->price , 2) }} DH</a> -->
                    <a href="#"
                        class="regular-btn inline-flex items-center justify-center font-medium text-white text-sm">{{ number_format($car->price , 2) }}
                        DH</a>
                </div>
            </form>
        </div>
    </div>
    <div class="product__grid grid  grid-cols-3 gap-y-6 xl:grid-cols-3 gap-x-6 mt-6">




        <div class="elem__product pl-3 md:pl-6 pr-3 py-4 bg-white flex items-center justify-start">
            <span class="inline-flex items-center justify-center  mr-4"><img src="{{ asset('img/icon3.svg')}}"
                    alt="icon"></span>
            <div class="product__info">
                ...
                <div class="product__double flex-wrap flex items-start sm:items-center justify-start">
                    <div class="el__double">
                        <p class="font-semibold text-lg sm:text-xl md:text-2xl ">{{ $car->number_view }} <span
                                class="text-sm font-semibold">views</span></p>
                    </div>
                    <span class="mx-2">/</span>
                    <div class="el__double">
                        <p class="font-semibold text-lg sm:text-xl md:text-2xl ">{{ $car->number_click }} <span
                                class="text-sm font-semibold">Clicks</span></p>
                    </div>
                </div>
            </div>
        </div>
